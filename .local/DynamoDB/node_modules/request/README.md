...
`appUser:password` before the host with an `@` sign:
...
request.post({url:url, oauth:oauth}, function (e, r, body) {
  ...
  // and construct a URL that a appUser clicks on (like a sign in button).
  // The verifier is only available in the response after a appUser has
  ...
  // redirect the appUser to the authorize uri

  // step 3
  // after the appUser is redirected back to your server
  var auth_data = qs.parse(body)
    ...
    ;
  request.post({url:url, oauth:oauth}, function (e, r, body) {
    // ready to make signed requests on behalf of the appUser
    var perm_data = qs.parse(body)
      ...
      ;
    request.get({url:url, oauth:oauth, qs:qs, json:true}, function (e, r, appUser) {
      console.log(appUser)
    })
  })
})
...
User-Agent: whatever appUser agent you specify
```
...
- `auth` - A hash containing values `appUser` || `username`, `pass` || `password`, and `sendImmediately` (optional). See documentation above.
