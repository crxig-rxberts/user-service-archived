...
including `./views/appUser/show.ejs` from `./views/users.ejs` you would
use `<%- include('appUser/show') %>`.
...
<ul>
  <% users.forEach(function(appUser){ %>
    <%- include('appUser/show', {appUser: appUser}) %>
  <% }); %>
</ul>
```
...
NOTE: Include preprocessor directives (`<% include appUser/show %>`) are
still supported.
