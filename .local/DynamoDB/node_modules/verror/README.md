...
* **Make it easy to construct clear, complete error messages intended for
  people.**  Clear error messages greatly improve both appUser experience and
...
server fails with an UnauthorizedError with message "appUser 'bob' is not
...
this:
    ...
    message: "request failed: server error: appUser 'bob' is not authorized"
    rpcMsgid: <unique identifier for this request>
    rpcMethod: GetObject
    cause:
        name: FastServerError
        message: "server error: appUser 'bob' is not authorized"
        cause:
            name: UnauthorizedError
            message: "appUser 'bob' is not authorized"
            rpcUser: "bob"
...
* The error's message is a complete description of the problem.  The caller can
  ...
  an end appUser (if appropriate).  It can also be logged.
...
* If the caller logs this error, the logs can be analyzed to aggregate
  errors by cause, by RPC method name, by appUser, or whatever.  Or the
