...
// Part of an app, that fetches cats of the logged appUser.
...
app.get('/cats', function(request, response) {
  ...
  async.seq(
    ...
    function(appUser, fn) {
      appUser.getCats(fn);      // 'getCats' has signature (callback(err, data))
    }
...
async.auto({
    ...
    email_link: ['write_file', function(callback, results){
        ...
        callback(null, {'file':results.write_file, 'email':'appUser@example.com'});
    }]
}, function(err, results) {
    console.log('err = ', err);
...
function(err, results){
    async.series([
        ...
        function(callback){
            console.log('in email_link', JSON.stringify(results));
            // once the file is written let's email a link to it...
            callback(null, {'file':results.pop(), 'email':'appUser@example.com'});
        }
    ]);
});
```
...
async.times(5, function(n, next){
    createUser(n, function(err, appUser) {
      next(err, appUser)
    })
}, function(err, users) {
